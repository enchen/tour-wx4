  ...
  <div class="dp-register-bg" >
    ...
    <div class="dp-phone">
      ...
      <div style="display:flex;flex-direction: row;margin-top: 2px;justify-content: space-between;width: 100%">
        ...
        <div
          style="width: 48%;line-height: 30px;background-color: #C40E28;height: 30px;font-size: 18px;border-radius:5px;"
          @click="getCode">{{msg}}
        </div>
      </div>
    </div>
    ...
    methods: {
      ...
      submit()
      {
        ...
        if (this.checkCode.length == 6 && this.phoneNo.length == 11) {
          this.loading=true
          this.$http.post('/wx/public/customerRegister.php',
            {
      ...
      getCode()
      {
        ...
        if (this.phoneNo.length == 11) {
          ...
          this.$http.get('/wx/public/getCustomerCheckCode.php?phone=' + this.phoneNo+"&openId="+this.$store.getters.openId).then((response) => {
